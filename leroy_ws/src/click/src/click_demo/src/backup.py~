...
def callback(data):
	...
	#obj.pose.position.x = data.position.x;
	#obj.pose.position.x = data.position.y;
	#obj.pose.position.x = data.position.z;
...
def listener():
	rospy.init_node('move_listener', anonymous = True, disable_signals=True)
